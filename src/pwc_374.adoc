...
which should be a stem:[O(m * log n)] operation, however a smart interpreter could do it in
stem:[O(m + log n)] given that the list of end points is sorted, so I'm going to say that's the 
...
and the first invalid position is right after the link:https://wiki.k-language.dev/wiki/Min[minimum (``&``)] of the previous
occurrences for the vowels. We link:https://wiki.k-language.dev/wiki/Enum[enumerate (``!``)] the valid beginning positions
