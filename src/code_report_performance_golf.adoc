...
footnote:[There were a couple revisions due to me both getting the direction of the 
comparison "wrong" and also forgetting that in Cpp `heap` refers to a max-heap,
so the errors cancelled out]
...
look at the indices to break ties in the values footnote:[And in fact the first draft of this
post also forgot to do break ties in the linear search of the second implementation].
...
void get_final_state(std::vector<uint64_t> &inout_vals, uint64_t k, uint64_t m)
{
	...
	while(k > std::distance(left, right))
	{
		...
		auto mid = right;
		right = std::find_if(mid, perm.end(), [&](auto i){ return cmp(*(mid-1), i); });
...
the second implementation is in fact a lot faster if stem:[k] and stem:[n]
are both the same order of magnitude and large.
The timings for stem:[n = k = 1'000'000, m = 3] and random elements in the 
range stem:[(0, n\]] are as follows:

[source]
----
heap-based:      1761457us
partition-based: 348003us
----
